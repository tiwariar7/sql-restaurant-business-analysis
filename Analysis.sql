...
-- Successfully Imported the CSV data in SQL DataBase
select * from restaurant_db.restaurants;

-- 1. Which restaurant of abohar is visied by least number of people?
select * from restaurant_db.restaurants where city = 'Pune' and rating_count = (select min(rating_count) from restaurants where city = 'Pune');

-- 2. Which restaurant has generated maximum revenue all over india?
select * from restaurant_db.restaurants where cost*rating_count = (select max(cost*rating_count) from restaurants);

-- 3. How many restaurants are having rating more than the average rating?
select * from restaurant_db.restaurants where rating > (select avg(rating) from restaurants);

-- 4. Which restaurant of Delhi has generated most revenue?
select * from restaurant_db.restaurants where city = 'Delhi' and 
